...
public class Click_on_Zone11_During_TimeBased_Irrigation extends Z11_Valve_Select
{
	@Test(priority = 4 , dependsOnMethods = { "select_Z11TimeBased" })
    public void click_Zone_During_SingleValve_Irrigation() throws AWTException, InterruptedException
    {
	...
	WebElement DuringIrrigation = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("(//img[@class='ng-star-inserted'])[11]")));
	...
	while (System.currentTimeMillis() < endTime || DuringIrrigation.getAttribute("class").contains("../../assets/new_assets/images/addfarmanimations.gif")) {
	    try {
	        ...
	        if (Seconds % 25 == 0) {
	            ...
	            System.out.println("Clicked on zone 11 icon at " + Seconds + " Seconds");
	        }
	    } catch (StaleElementReferenceException e) {
