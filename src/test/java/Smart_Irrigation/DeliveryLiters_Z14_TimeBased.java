...
public class DeliveryLiters_Z14_TimeBased extends Z14_Valve_Select
{
	@Test(priority = 4 , dependsOnMethods = { "SelectZ14_valve_TimeBased" })
    public void DeliveryLiters_z14_SingleValve_TimeBased() throws AWTException, InterruptedException
    {
	...
	while (System.currentTimeMillis() < endTime || DuringIrrigation.getAttribute("class").contains("../../assets/new_assets/images/addfarmanimations.gif"))
	{
	    ...
	    DuringIrrigation = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("(//img[@class='ng-star-inserted'])[14]")));

	    String deliveryliters = deliverdLitersElement.getText();
	    System.out.println("Delivered liters  on zone 14 at " + Seconds + " Seconds: " + deliveryliters);
